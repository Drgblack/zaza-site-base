  ...
  return (
    <div className={`${variantClasses[variant]} p-6 ${sizeClasses[size]} ${className}`}>
      <div className="text-center mb-4">
        ...
        <h3 className={`font-bold text-gray-800 mb-2 ${size === 'lg' ? 'text-2xl' : size === 'md' ? 'text-xl' : 'text-lg'}`}>{title}</h3>
        <p className={`text-gray-600 ${size === 'lg' ? 'text-lg' : size === 'md' ? 'text-base' : 'text-sm'}`}>{subtitle}</p>
      </div>
